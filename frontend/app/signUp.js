...
export default function SignUp() {
    ...
    const handleRegister = async () => {
        if (!emailRef.current || !passwordRef.current || !usernameRef.current || !profileRef.current || !selectedLanguage) {
            Alert.alert('Sign Up', "Please fill all the fields!");
            return;
        }
        setLoading(true);

        let response = await register(emailRef.current, passwordRef.current, usernameRef.current, profileRef.current, selectedLanguage);
        setLoading(false);
            ...
            <View style={{ paddingTop: hp(7), paddingHorizontal: wp(5) }} className="flex-1 gap-12">
                ...
                <View className="gap-10">
                    ...
                    <View className="gap-4">
                        ...
                        {/* Language Selector */}
                        <View style={{ height: hp(18) }} className="flex-row gap-4 px-4 bg-neutral-100 items-center rounded-xl">
                            <Feather name="globe" size={hp(2.7)} color="gray" />
                            <Picker
                                selectedValue={selectedLanguage}
                                onValueChange={(itemValue, itemIndex) => setSelectedLanguage(itemValue)}
                                style={{ flex: 1, fontSize: hp(2) }}
                                itemStyle={{ fontSize: hp(2) }}
                                pickerStyleType={{ fontSize: hp(2) }}
                            >
